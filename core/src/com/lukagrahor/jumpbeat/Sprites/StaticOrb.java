...
public class StaticOrb extends StaticItems {
    ...
    public StaticOrb(PlayScreen screen, float x, float y, Hud hud) {
        super(screen, x, y,hud);
        frames = new Array<TextureRegion>();
        for(int i = 0; i < 7; i++){
            frames.add(new TextureRegion(screen.getAtlas2().findRegion("OrbAnimation-Sheet"),i*14,0,14,13));
        }
        ...
        manager.load("audio/sounds/pickup.wav", Sound.class);
        manager.finishLoading();
        sound = manager.get("audio/sounds/pickup.wav",Sound.class);
    }
    ...
    public void update(float dt){
        ...
        if(setToDestroy && !destroyed) {
            ...
            destroyed = true;
            setRegion(new TextureRegion(screen.getAtlas2().findRegion("OrbAnimation-Sheet"),0,0,14,13));
            stateTime = 0;
        }
        else if(!destroyed) {
    ...
    protected void defineStaticItems() {
        ...
        fdef.filter.categoryBits = JumpBeatClass.ORB_BIT;
